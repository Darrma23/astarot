...
export default async function printInfo(m, conn = { user: {} }) {
		...
		// 🔥 FINAL OUTPUT: Clean & Simple Logger
		// STYLE BOX LOGGING
		const title = `${subject || chatContext} • ${chatContext}`;
		const number = phoneNumber.split("@")[0];
		console.log(`${colorANSI}┌─ [${time}] ${title}${reset}\n${colorANSI}│ Sender : ${user} (${number})${reset}\n${colorANSI}│ Type   : ${idFormat}${reset}\n${colorANSI}│ Msg    : ${pesan}${reset}\n${colorANSI}└────────────────────────────\n${reset}`);
}
